...
class TestShellTools(unittest.TestCase):
    ...
    def setup_remote_and_clone(self):
        ...
        subprocess.check_call(["git", "remote", "add", "origin", str(origin_path)], cwd=setup_path)
        subprocess.check_call(["git", "push", "-u", "origin", "main"], cwd=setup_path)
        
        # Clone it
        local_path = Path(self.test_dir) / "local"
        subprocess.check_call(["git", "clone", str(origin_path), "local"], cwd=self.test_dir)
    ...
    def test_wta_create_new_branch(self):
        ...
        full_command = f"export HOME={fake_home} && source {INIT_SCRIPT} && wta new-feature"
        ...
        res_wt = subprocess.run(["git", "worktree", "list"], cwd=self.test_dir, capture_output=True, text=True)
    ...
    def test_gupdate(self):
        ...
        res_log = subprocess.run(["git", "log", "--oneline"], cwd=local_path, capture_output=True, text=True)
    ...
    def test_grestack(self):
        ...
        subprocess.check_call(["git", "add", "."], cwd=setup_path)
        subprocess.check_call(["git", "commit", "-m", "Squashed parent"], cwd=setup_path)
        subprocess.check_call(["git", "push", "origin", "main"], cwd=setup_path)
        ...
        log = subprocess.run(["git", "log", "--oneline"], cwd=local_path, capture_output=True, text=True)
